...
def analyze_geo_with_ai(page_data: dict) -> dict:
    ...
    # 5 realistic consumer queries — the kind people actually ask ChatGPT
    queries = [
    ...
    # Call 1: Get 5 real answers — NO brand bias, GPT answers naturally
    ...
    full_text = " ".join([a["a"] for a in qa_pairs]).lower()
    brand_l   = brand.lower()

    # Call 2: Objective scoring
    qa_parts = []
    for i, p in enumerate(qa_pairs):
        qa_parts.append(f"Q{i+1}: {p['q']}\nA{i+1}: {p['a']}")
    qa_formatted = "\n\n".join(qa_parts)

    scoring_prompt = f"""You are an objective GEO measurement analyst. Read these 5 AI responses about "{brand}" and score each metric ACCURATELY based ONLY on what the responses actually say.
...
VISIBILITY (0-100):
Count how many of the 5 responses mention "{brand}" by name.
- 5/5 mentioned = 100
- 4/5 = 80
- 3/5 = 60
- 2/5 = 40
